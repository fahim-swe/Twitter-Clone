...
export class TweetViewComponentStore extends ComponentStore<State>{
    constructor(private snackbar:MatSnackBar,private loadingService:LoadingService,private tweetActionService : AbsTweetActionService,private tweetService:AbsTweetService) {
        super({tweet:{},parentTweet:{}})
     }

     updateParentTweet = this.updater((state,parentTweet:TweetModel)=>({...state,parentTweet:parentTweet}))
     patchParentTweet = this.updater((state,parentTweet:TweetModel)=>({...state,parentTweet:{...state.parentTweet,...parentTweet}}))
     ...
     retweet = this.effect((id$:Observable<string>) => {
        ...
        this.loadingService.setLoading(true)
                return this.tweetActionService.retweet(id).pipe(tap(_=>{
         this.snackbar.open("Retweeted successfully","ok",{duration:2000}) 
