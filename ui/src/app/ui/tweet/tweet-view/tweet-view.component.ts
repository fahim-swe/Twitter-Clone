...
export class TweetViewComponent implements OnInit,OnChanges {
  ...
  like(tweet: TweetModel) {
    if (tweet.isLiked) {
      this.store.patchParentTweet({ totalLikes: tweet.totalLikes - 1 });
      this.store.unlike(tweet.id)
    } else {
      this.store.patchParentTweet({ totalLikes: tweet.totalLikes + 1 });
      this.store.like(tweet.id)
    }
    this.store.patchParentTweet({ isLiked: !tweet.isLiked });
  }
  comment(tweet: TweetModel) {
    if (this.commentText) {
      this.store.patchParentTweet({ totalComments: tweet.totalComments + 1 });
  ...
  retweet(tweet: TweetModel) {
    this.store.patchParentTweet({ totalRetweets: tweet.totalRetweets + 1 });
