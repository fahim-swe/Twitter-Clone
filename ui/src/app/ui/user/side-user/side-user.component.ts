...
export class SideUserComponent implements OnInit {
  ...
  this.users$ = httpService.get(ApiEndpoints.USERS+"/get-users",new HttpParams().append('PageSize',Math.floor(window.innerHeight/128)+2)).pipe(map((res:PageResponse<UserModel[]>)=>res.data))
  }
